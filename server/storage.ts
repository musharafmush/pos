  ...
  async createSaleWithItems(saleData: any, items: any[]): Promise<any> {
    try {
      ...
      const result = sqlite.transaction(() => {
        ...
        // Get the created sale
        const getSale = sqlite.prepare('SELECT * FROM sales WHERE id = ?');
        const newSale = get```tool_code
<replit_final_file>
This code implements the Open Register with Sales Dashboard Data Sync feature, including register management endpoints, storage methods, and database table creation.
import { db } from "../db/sqlite-index";
import {
  users,
  products,
