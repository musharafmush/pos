...
app.post("/api/customers", async (req, res) => {
  ...
  // Data Management & Backup API endpoints
  app.get('/api/backup/data-stats', isAuthenticated, async (req, res) => {
    try {
      const { sqlite } = await import('@db');
      
      // Get data statistics
      const stats = {
        products: sqlite.prepare('SELECT COUNT(*) as count FROM products').get()?.count || 0,
        categories: sqlite.prepare('SELECT COUNT(*) as count FROM categories').get()?.count || 0,
        suppliers: sqlite.prepare('SELECT COUNT(*) as count FROM suppliers').get()?.count || 0,
        customers: sqlite.prepare('SELECT COUNT(*) as count FROM customers').get()?.count || 0,
        sales: sqlite.prepare('SELECT COUNT(*) as count FROM sales').get()?.count || 0,
        purchases: sqlite.prepare('SELECT COUNT(*) as count FROM purchases').get()?.count || 0,
        users: sqlite.prepare('SELECT COUNT(*) as count FROM users').get()?.count || 0,
        
        // Get total revenue
        totalRevenue: sqlite.prepare('SELECT COALESCE(SUM(total), 0) as total FROM sales').get()?.total || 0,
        
        // Get database size (approximate)
        lastBackup: new Date().toISOString(),
        dbSize: '2.5 MB' // Placeholder for now
      };
  ...
  app.post('/api/backup/create', isAuthenticated, async (req, res) => {
    try {
      ...
      const filename = `awesome-shop-backup-${timestamp}.json`;
    ...
    try {
      ...
      upload.single('backup')(req, res, async (err: any) => {
        if (err) {
          ...
          return res.status(400).json({ error: 'Failed to upload backup file' });
        }
        ...
        try {
          ...
          const restoreTransaction = sqlite.transaction(() => {
            ...
            // Restore data
            const tables = ['categories', 'suppliers', 'customers', 'products', 'sales', 'sale_items', 'purchases', 'purchase_items'];
            ...
            for (const table of tables) {
              if (backupData.data[table] && Array.isArray(backupData.data[table])) {
                for (const record of backupData.data[table]) {
                  ...
                  sqlite.prepare(insertQuery).run(...values);
                }
              }
            }
          });
          ...
          res.status(400).json({ error: 'Invalid backup file format or corrupted data' });
        }
      });
    } catch (error) {
  ...
  app.post('/api/backup/clear-data', isAuthenticated, async (req, res) => {
    try {
      const { sqlite } = await import('@db');
      const { dataTypes } = req.body;

      if (!dataTypes || !Array.isArray(dataTypes)) {
        return res.status(400).json({ error: 'Data types array is required' });
      }

      // Start transaction for data clearing
      const clearTransaction = sqlite.transaction(() => {
      ...
      res.json({ 
        success: true, 
        message: `Successfully cleared ${dataTypes.join(', ')} data`,
