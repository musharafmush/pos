...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Record new payment for purchase
  app.post('/api/purchases/:id/payment', isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const { amount, method, date, notes, transactionId } = req.body;

      console.log('💰 Recording payment for purchase:', id, { amount, method, date });
      ...
      const purchase = sqlite.prepare('SELECT * FROM purchases WHERE id = ?').get(id);
      if (!purchase) {
      ...
      if (newPaidAmount > totalAmount) {
        return res.status(400).json({ 
          ...
          message: `Payment amount exceeds remaining balance of ₹${(totalAmount - currentPaidAmount).toFixed(2)}`
        });
      }
