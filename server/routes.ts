...
app.post("/api/customers", async (req, res) => {
  ...
  // Accounts Dashboard API endpoints
  app.get('/api/accounts/stats', async (req, res) => {
    try {
      console.log('📊 Fetching accounts statistics');
      ...
      const todaySales = await storage.listSales({ search: '', limit: 1000 });
      const todayTransactions = todaySales.filter(sale => 
        sale.createdAt && sale.createdAt.startsWith(today)
      );
      
      // Fetch purchase data for expenses calculation
      const purchases = await storage.listPurchases({ limit: 1000 });
      const todayPurchases = purchases.filter(purchase => 
        purchase.createdAt && purchase.createdAt.startsWith(today)
      );
      ...
      const totalExpenses = todayPurchases.reduce((sum, purchase) => 
      ...
      const stats = {
        ...
        totalAccounts: 4, // Cash, Bank, Digital, Credit
        todayTransactions: todayTransactions.length + todayPurchases.length,
        cashFlow: totalRevenue - totalExpenses,
        averageTransaction: todayTransactions.length > 0 ? totalRevenue / todayTransactions.length : 0,
        topAccount: 'Main Cash Register'
      };
      ...
      res.status(500).json({ error: 'Failed to fetch accounts statistics' });
    }
  });
  ...
  app.get('/api/accounts', async (req, res) => {
    try {
      ...
      // Get sales and purchase data to calculate balances
      const sales = await storage.listSales({ search: '', limit: 1000 });
      const purchases = await storage.listPurchases({ limit: 1000 });
      
      const cashSales = sales.filter(sale => sale.paymentMethod === 'cash');
      const cardSales = sales.filter(sale => sale.paymentMethod === 'card' || sale.paymentMethod === 'credit_card');
      const digitalSales = sales.filter(sale => sale.paymentMethod === 'upi' || sale.paymentMethod === 'digital');
      
      const cashRevenue = cashSales.reduce((sum, sale) => sum + parseFloat(sale.total), 0);
      const cardRevenue = cardSales.reduce((sum, sale) => sum + parseFloat(sale.total), 0);
      const digitalRevenue = digitalSales.reduce((sum, sale) => sum + parseFloat(sale.total), 0);
      const totalExpenses = purchases.reduce((sum, purchase) => sum + parseFloat(purchase.totalAmount), 0);
      ...
      const accounts = [
        {
          ...
          type: 'cash',
          balance: cashRevenue,
          lastTransaction: cashSales.length > 0 ? cashSales[0].createdAt : 'No transactions',
        ...
        },
        {
          ...
          type: 'bank',
          balance: cardRevenue - totalExpenses,
          lastTransaction: cardSales.length > 0 ? cardSales[0].createdAt : 'No transactions',
          transactionCount: cardSales.length + purchases.length,
          status: 'active'
        },
        {
          id: 3,
          name: 'Digital Wallet',
          type: 'digital',
          balance: digitalRevenue,
          lastTransaction: digitalSales.length > 0 ? digitalSales[0].createdAt : 'No transactions',
        ...
        },
        {
          ...
          name: 'Credit Account',
          type: 'credit',
          balance: -totalExpenses * 0.2, // 20% on credit
          lastTransaction: purchases.length > 0 ? purchases[0].createdAt : 'No transactions',
          transactionCount: Math.floor(purchases.length * 0.2),
          status: 'active'
        }
      ];
  ...
  app.get('/api/accounts/transactions', async (req, res) => {
    try {
      const { period = 'today', account = 'all' } = req.query;
      console.log('📊 Fetching account transactions for period:', period, 'account:', account);
      ...
      for (const sale of sales) {
        ...
        if (saleDate >= startDate) {
          let accountName = 'Main Cash Register';
          if (sale.paymentMethod === 'card' || sale.paymentMethod === 'credit_card') {
            accountName = 'Business Bank Account';
          } else if (sale.paymentMethod === 'upi' || sale.paymentMethod === 'digital') {
            accountName = 'Digital Wallet';
          }
          
          if (account === 'all' || accountName === account) {
            transactions.push({
              id: `sale-${sale.id}`,
              type: 'sale',
      ...
      for (const purchase of purchases) {
        ...
        if (purchaseDate >= startDate) {
          const accountName = 'Business Bank Account';
  ...
  app.get('/api/accounts/cash-flow', async (req, res) => {
    try {
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        const daySales = sales.filter(sale => 
          sale.createdAt && sale.createdAt.startsWith(dateStr)
        );
        
        // Get purchases for this date
        const purchases = await storage.listPurchases({ limit: 1000 });
        const dayPurchases = purchases.filter(purchase => 
          purchase.createdAt && purchase.createdAt.startsWith(dateStr)
        );
        
        const inflow = daySales.reduce((sum, sale) => sum + parseFloat(sale.total), 0);
        const outflow = dayPurchases.reduce((sum, purchase) => sum + parseFloat(purchase.totalAmount), 0);
      ...
      console.log('📊 Generated cash flow data for', cashFlowData.length, 'days');
  ...
  app.post('/api/accounts', async (req, res) => {
    try {
      ...
      console.log('📊 Creating new account:', { name, type, initialBalance });
      
      // For now, return success (in a real implementation, this would save to database)
      const newAccount = {
        id: Date.now(),
        name,
        type,
