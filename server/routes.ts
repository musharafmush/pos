...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.put('/api/purchases/:id', isAuthenticated, async (req, res) => {
    try {
      const purchaseId = parseInt(req.params.id);
      const { 
        supplierId, orderNumber, orderDate, expectedDate, paymentTerms, 
        paymentMethod, status, priority, shippingAddress, billingAddress,
        shippingMethod, freightAmount, surchargeAmount, packingCharges,
        otherCharges, additionalDiscount, taxAmount, discountAmount,
        invoiceNumber, invoiceDate, invoiceAmount, lrNumber, remarks,
        internalNotes, total, items 
      } = req.body;
      ...
      const updatedPurchase = await storage.updatePurchase(purchaseId, updateData);
      ...
      if (items && Array.isArray(items)) {
        await storage.updatePurchaseItems(purchaseId, items);
      }
  ...
  app.delete('/api/purchases/:id', isAuthenticated, async (req, res) => {
    try {
      ...
      await storage.deletePurchaseItems(purchaseId);
      
      // Delete the purchase
      const deleted = await storage.deletePurchase(purchaseId);
