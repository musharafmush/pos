...
import { HSNSelector } from "@/components/hsn-selector";
                  ...
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2">
                        ...
                        Tax Information & HSN Management
                      </CardTitle>
                      <CardDescription>
                        Configure HSN codes and GST rates. HSN codes sync between Add Item and Settings.
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6">
                      {/* HSN Code Selection with Comprehensive Management */}
                      <div className="col-span-2">
                        <HSNSelector
                          selectedHSN={form.watch("hsnCode")}
                          onHSNSelect={(hsnCode, taxRates) => {
                            // Update form with selected HSN and tax rates
                            form.setValue("hsnCode", hsnCode);
                            form.setValue("cgstRate", taxRates.cgst);
                            form.setValue("sgstRate", taxRates.sgst);
                            form.setValue("igstRate", taxRates.igst);
                            form.setValue("cessRate", taxRates.cess);
                            
                            // Calculate and display total GST
                            const totalGst = parseFloat(taxRates.igst) || (parseFloat(taxRates.cgst) + parseFloat(taxRates.sgst));
                            form.setValue("gstCode", `GST ${totalGst}%`);
                          }}
                          onNewHSNRequest={(hsnCode) => {
                            // Handle request to save new HSN code to database
                            console.log("New HSN code requested for addition:", hsnCode);
                          }}
                        />
                      </div>
                      ...
                      <div className="border-t pt-6">
                        ...
                        <div className="mt-4 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                          ...
                          <ul className="text-sm text-yellow-700 space-y-1">
                            <li>• HSN codes are automatically synced between Add Item and Settings</li>
                            <li>• CGST + SGST applies for intra-state transactions</li>
                            <li>• IGST applies for inter-state transactions</li>
                            <li>• Cess is additional tax on specific goods (tobacco, luxury cars)</li>
                          </ul>
                        </div>
                      </div>
                    </CardContent>
...
export default AddItemProfessional;